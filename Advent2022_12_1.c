...
int main(int Arguments, char* Argument[])
{
	...
	///////////////////////////    Get one full line of chars     ///////////////////////////
		...
		for (InputPos=0; InputPos<InputLen; InputPos++)
		{
			...
			if (InputLine[InputPos] == 'E')
			{
				...
				EndX = InputPos;  EndY=InputLineNr;
				continue;
			}
  ...
  printf("Optimising path from Start[%d,%d] to End[%d,%d]\n", StartX,StartY, EndX,EndY);
  ...
  int ExtraX[MAX_EXTRA],ExtraY[MAX_EXTRA], NrOfExtra=0;
	...
	for (; NrOfNew>0;)
	{
        ...
        if ((Cost[DestX][DestY]) && (Cost[DestX][DestY] <= Cost[X][Y]+1))
          continue;
        // - this Choice requires too steep a climb
	...
	// To know the cost of the End cell, just look it up -
	//   but don't forget to subtract the fake cost=1 of the Start cell.
