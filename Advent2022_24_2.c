  ...
  // cells of the grid at a certain Round.
    ...
    for (TrailNr=0; TrailNr<NrOfTrails; TrailNr++)
    {
      ...
      for (Choice=-1; Choice<4; Choice++)
      {
        ...
        // 1. Having reached a solution
        if ((Choice >= 0) &&
            ((((Pass==0) || (Pass==2)) && (NewPosX == SizeX-1) && (NewPosY == SizeY)) ||
                ((Pass==1) && (!NewPosX) && (NewPosY == -1))))
        {
          printf("Minimum trail found in Round %d\n", Round+1);
          break;
        }
    ...
    if (Choice < 4)
    {
      // Throw away all Trails, except for this successful winner of the search
      TrailX[0]=NewPosX; TrailY[0]=NewPosY; NrOfTrails=1;
      if (++Pass > 2) break;
    }
    else
    {
