...
int main(int Arguments, char* Argument[])
{
	...
	for (Round=0; Round<10; Round++)
	{
		...
		/* Reporting */
		/*************/
		// Find the bounding box around the current collection of Elves
		int MinX, MaxX, MinY, MaxY;
		for (X=0; X<300; X++)
		{
			for (Y=0; Y<300; Y++)  if (Field[X][Y].Content == '#')  break;
			if (Y<300) { MinX = X; break; }
		}
		for (X=300-1; X; X--)
		{
			for (Y=0; Y<300; Y++)  if (Field[X][Y].Content == '#')  break;
			if (Y<300) { MaxX = X; break; }
		}
		for (Y=0; Y<300; Y++)
		{
			for (X=0; X<300; X++)  if (Field[X][Y].Content == '#')  break;
			if (X<300) { MinY = Y; break; }
		}
		for (Y=300-1; Y; Y--)
		{
			for (X=0; X<300; X++)  if (Field[X][Y].Content == '#')  break;
			if (X<300) { MaxY = Y; break; }
		}
		// Display this bounding box, and count the empty fields
		int NrOfEmpty = 0;
		for (Y=MinY; Y<=MaxY; Y++)
		{
			for (X=MinX; X<=MaxX; X++)
			{
				if (1)  putchar(Field[X][Y].Content);
				if (Field[X][Y].Content == '.')  NrOfEmpty++;
			}
			if (1)  putchar('\n');
		}
		if (1)  printf("After Round %d, %d empty fields within bounding box.\n", Round+1, NrOfEmpty);
