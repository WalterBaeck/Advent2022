...
#define CIRCLE_MAX 100
  ...
  // Now, consider what happens when delegating part of the nonzero-Flow valves to the elephant
  // As the human and the elephant are interchangable, all 2**NrOfVisit combinations would entail double work.
  // Therefore, the topmost bit will forever remain the prerogative of the human.
  int MaxSum=0;
  for (int ElephantMask=0; ElephantMask<(1<<(NrOfVisit-1)); ElephantMask++)
  {
    int PlayerValve[2][20],NrOfPlayerValves[2]={0};
    for (int BitNr=0; BitNr<NrOfVisit; BitNr++)
      if (ElephantMask & (1<<BitNr))
        PlayerValve[0][NrOfPlayerValves[0]++] = Visit[BitNr];
      else
        PlayerValve[1][NrOfPlayerValves[1]++] = Visit[BitNr];
    
    // The puzzle remains the same for each player with his subset of valves to Visit
    int MaxScore[2]={0};
    for (int Player=0; Player<2; Player++)
    {
      // Go over all possible orderings to visit these Flow valves
      int Time, Score;
      InitPermutation(Choice, NrOfPlayerValves[Player]);
      ...
      for(;;)
      {
        ...
        for (VisitNr=0; VisitNr<NrOfPlayerValves[Player]; VisitNr++)
        {
          // CurrentValveNr = ValveNr
          // TargetValveNr = PlayerValve[Player][Choice[VisitNr]]
          ...
          for (SearchNr=0; SearchNr<NrOfDistance; SearchNr++)
            if (((DistFrom[SearchNr] == ValveNr) && (DistTo[SearchNr] == PlayerValve[Player][Choice[VisitNr]]))
                || ((DistTo[SearchNr] == ValveNr) && (DistFrom[SearchNr] == PlayerValve[Player][Choice[VisitNr]])))  break;
          if (SearchNr >= NrOfDistance)
          {
            fprintf(stderr, "Internal error: distance from %s[%d] to %s[%d] not found in %d distances\n",
                Valve[ValveNr].Name, ValveNr,
                Valve[PlayerValve[Player][Choice[VisitNr]]].Name, PlayerValve[Player][Choice[VisitNr]],
                NrOfDistance);
            exit(4);
          }
          ...
          Score += (26-Time) * Valve[PlayerValve[Player][Choice[VisitNr]]].Flow;
          // The place where we now find ourselves, becomes the new starting point
          ValveNr = PlayerValve[Player][Choice[VisitNr]];
        } /* for (VisitNr) */

        if (Score > MaxScore[Player])  MaxScore[Player] = Score;

        if (VisitNr < NrOfPlayerValves[Player])
        {
          ...
          if (!(NextPermutationFromPos(Choice, NrOfPlayerValves[Player], VisitNr)))
            break;
        }
        else
        {
          // Regular increase of the PermutationNr
          if (!(NextPermutation(Choice, NrOfPlayerValves[Player])))
            break;
        }
      } /* for(;;) */
    } /* for (Player) */
    if (MaxScore[0]+MaxScore[1] > MaxSum)  MaxSum = MaxScore[0]+MaxScore[1];
  } /* for (ElephantMask) */
